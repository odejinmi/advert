...
class Freemoney {
  ...
  // Constructor
  Freemoney(this._adUnitIds, this._reporter) {
    preloadAds();
  }
  ...
  void _loadNextAd() {
    if (_adUnitIds.isEmpty) {
      debugPrint('No ad unit IDs provided for Freemoney');
      return;
    }

    if (_currentLoadingIndex >= _adUnitIds.length) {
      _currentLoadingIndex = 0; // wrap
    }
    ...
    debugPrint(
        'Loading freemoney ad ${_currentLoadingIndex + 1}/${_adUnitIds.length}');
  ...
  void _configureAndShowAd(
    ...
    ad.fullScreenContentCallback = FullScreenContentCallback(
      ...
      onAdImpression: (RewardedAd ad) {
        debugPrint('Freemoney ad impression');
